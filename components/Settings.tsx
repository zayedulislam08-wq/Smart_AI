...
import { GEMINI_MODELS, SERVICES, ServiceId } from '../constants';
...
// ─── Feature → Service Name Map ──────────────────────────────────────────────
const FEATURE_LABELS: Record<string, string> = {
    chatbot: 'AI Chatbot',
    fast_chat: 'Low-Latency Chat',
    live_conversation: 'Live Conversation',
    image_generator: 'Image Generation',
    image_editor: 'Image Editor',
    image_analyzer: 'Image Analyzer',
    grounding_search: 'Web & Maps Search',
    text_to_speech: 'Text-to-Speech',
    audio_transcriber: 'Audio Transcription',
    complex_task: 'Thinking Mode',
...
// Models available for each type of feature
const MODELS_BY_FEATURE: Record<string, string[]> = {
    chatbot:            ['gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-2.5-flash-lite'],
    fast_chat:          ['gemini-2.5-flash-lite', 'gemini-2.5-flash'],
    live_conversation:  ['gemini-2.5-flash-native-audio-preview-09-2025', 'gemini-2.5-flash-native-audio-preview'],
    image_generator:    ['imagen-4.0-generate-001', 'imagen-4.0-fast-generate-001', 'imagen-4.0-ultra-generate-001', 'imagen-3.0-generate-001'],
    image_editor:       ['gemini-2.5-flash-image'],
    image_analyzer:     ['gemini-2.5-flash', 'gemini-2.5-pro'],
    grounding_search:   ['gemini-2.5-flash', 'gemini-2.5-pro'],
    text_to_speech:     ['gemini-2.5-flash-preview-tts'],
    audio_transcriber:  ['gemini-2.5-flash', 'gemini-2.5-pro'],
    complex_task:       ['gemini-2.5-pro', 'gemini-2.5-flash'],
    document_processor: ['gemini-2.5-pro', 'gemini-2.5-flash'],
    video_analyzer:     ['gemini-2.5-pro', 'gemini-2.5-flash'],
    floating_chat:      ['gemini-2.5-flash', 'gemini-2.5-pro'],
    workflow:           ['gemini-2.5-pro', 'gemini-2.5-flash'],
};
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    {editing ? (
                        <input
                            autoFocus
                            className="text-sm font-semibold text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800 border border-blue-400 rounded-lg px-2 py-1 outline-none w-40"
                            value={nameInput}
...
export const Settings: React.FC = () => {
    ...
    const handleAddKey = () => {
        ...
        if (!newKeyValue.trim() || !newKeyValue.startsWith('AIza')) { setAddError('Invalid API key format. Keys start with "AIza".'); return; }
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 p-5 space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <input
                                className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                                placeholder='Key name (e.g. "Personal Key")'
                                value={newKeyName}
                                onChange={e => { setNewKeyName(e.target.value); setAddError(''); }}
                            />
                            <input
                                className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition font-mono"
                        ...
                        <p className="text-[11px] text-gray-400 dark:text-gray-500">
                            ...
                            . When a key hits its rate limit, the system will auto-rotate to the next available key.
                        </p>
                    </div>
                        ...
                        <div className="space-y-3">
                            {apiKeys.map(k => (
                                <ApiKeyCard
                                    key={k.id}
                                    apiKey={k}
                                    onRemove={() => removeApiKey(k.id)}
                                    onRename={name => updateApiKey(k.id, { name })}
                                />
                            ))}
                        </div>
                    )}
            ...
            {activeTab === 'models' && (
                <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5">
                        <h2 className="font-bold text-gray-900 dark:text-white text-sm">Per-Feature Model & Key Assignment</h2>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Choose which Gemini model and API key each feature uses.</p>
                    </div>
                    <div className="divide-y divide-gray-50 dark:divide-white/5">
                        {Object.entries(FEATURE_LABELS).map(([featureId, label]) => {
                            const config = featureConfigs.find(f => f.featureId === featureId);
                            const availableModels = MODELS_BY_FEATURE[featureId] || ['gemini-2.5-flash'];
                            const currentModel = config?.modelId || availableModels[0];
                            const currentKeyId = config?.apiKeyId || 'auto';

                            return (
                                <div key={featureId} className="flex flex-col sm:flex-row items-start sm:items-center gap-3 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-white/2 transition-colors">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{label}</p>
                                        <p className="text-[11px] text-gray-400">{featureId}</p>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                                        {/* Model selector */}
                                        <select
                                            value={currentModel}
                                            onChange={e => setFeatureConfig(featureId, e.target.value, currentKeyId)}
                                            className="text-xs bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
                                        >
                                            {availableModels.map(m => (
                                                <option key={m} value={m}>{GEMINI_MODELS[m]?.label || m}</option>
                                            ))}
                                        </select>
                                        ...
                                        <select
                                            value={currentKeyId}
                                            onChange={e => setFeatureConfig(featureId, currentModel, e.target.value)}
